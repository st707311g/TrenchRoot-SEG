...
def get_train_generator(seed=1, batch_size = 10):
    ...
    mask_generator = mask_datagen.flow_from_directory(IN_DIR, classes = [Y_NAME], seed=seed, batch_size = batch_size, class_mode=None, color_mode = 'grayscale')

    train_generator = zip(image_generator, mask_generator)

    for ximg, yimg in train_generator:
        ximg = np.array(ximg, dtype=np.float32)/255
        yimg = (np.array(yimg) >= 128)*1.
        yield (ximg,yimg)
